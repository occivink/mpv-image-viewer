...
-- msg.info("center	= " .. tostring(opts.center))
...
local function refresh_minimap()
  ...
  local draw = function(x, y, w, h, opacity, color)
    ...
    if rounded[1] then
         a:line_to(x1 - r, y0    ); a:bezier_curve(x1 - r + c, y0        , x1        , y0 + r - c, x1    , y0 + r)
    else a:line_to(x1    , y0    ) end
    if rounded[2] then
         a:line_to(x1    , y1 - r); a:bezier_curve(x1        , y1 - r + c, x1 - r + c, y1        , x1 - r, y1    )
    else a:line_to(x1    , y1    ) end
    if rounded[3] then
         a:line_to(x0 + r, y1    ); a:bezier_curve(x0 + r - c, y1        , x0        , y1 - r + c, x0    , y1 - r)
    else a:line_to(x0    , y1    ) end
    if rounded[4] then
         a:line_to(x0    , y0 + r); a:bezier_curve(x0        , y0 + r - c, x0 + r - c, y0        , x0 + r, y0    )
    else a:line_to(x0    , y0    ) end
    a:draw_stop()
  end
  local image = function()
    draw(
      (     dim.ml/2 - dim.mr/2) / opts.scale,
