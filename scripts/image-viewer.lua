...
function align_border(x, y)
    compute_video_dimensions()
    local video_h, video_w = video_dimensions.size.h, video_dimensions.size.w
    local window_w, window_h = mp.get_osd_size()
    local x,y = tonumber(x), tonumber(y)
    local command = ""
    if x then
        command = command .. "no-osd set video-pan-x " .. x * (video_w - window_w) / (2 * video_w) .. ";"
    end
    if y then
        command = command .. "no-osd set video-pan-y " .. y * (video_h - window_h) / (2 * video_h) .. ";"
    end
    if command ~= "" then
        mp.command(command)
    end
