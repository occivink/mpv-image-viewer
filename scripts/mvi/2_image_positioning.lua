...
function align_border(x, y)
  ...
  local dim = mp.get_property_native("osd-dimensions")
  local dimw = dim.w
  -- if not dim.w then print("returning on zero dim.w=¦"..tostring(dim.w)); return; end
  -- if not dim then return end
  print("dimw=¦"..tostring(dimw).."¦")
  if dimw==0 then
    print("returning on zero dimw=¦"..dimw.."¦")
    return
  else
    print("not returning no nonzero dimw=¦"..dimw.."¦")
  end
